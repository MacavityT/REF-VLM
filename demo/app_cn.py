...
def img_change_sketch(input_image: dict):
    # input_image['mask'] = input_image['composite'] - input_image['background']
    return input_image
...
def submit_step1(input_text, input_image, chatbot, radio, state, prompt_image_list, point_mask=None):
    ...
    if isinstance(input_image,dict):
        ...
        if input_text.count(MASKS_PLACEHOLDER) == 1:
            sketch = input_image['composite'] - input_image['background']
            sketch = (sketch[:,:,0]).astype(np.uint8)
            sketch = np.where(sketch > 0,1,0).astype(np.uint8)
            cur_prompt_image_list.append(sketch)
        else:
    ...
    if input_image is not None:
        if isinstance(input_image,np.ndarray):
            ...
            state['input_data'].add_image(input_image['background'])
...
# title_markdown = ("""
# ![LOGO](/code/VT-PLUG/demo/assets/logo/yongzhi-logo.png)
# # 🌋 甬知通用多模态大模型
# """)
...
with gr.Blocks(
    ...
    with gr.Row():
        ...
        #     <p align="center">
        #     <a href="#">
        #     <img src="https://ice.frostsky.com/2025/01/08/c86100db476fb7c453378434bb35a981.png" alt="Logo" width="200"></a>
        #     </p>
        ...
        gr.HTML(
            ...
            <p align="center">
            <a href="#">
            <img src="https://ice.frostsky.com/2025/01/08/c86100db476fb7c453378434bb35a981.png" alt="Logo" width="800"></a>
            </p>
            """
        )
    ...
    with gr.Row():
        with gr.Column():
            with gr.TabItem("输入图像"):
                with gr.Row():
                    with gr.Column():
                        input_img_1 = gr.Image(type="numpy", label="输入图像", height=550,image_mode='RGB')
                        
                        radio_1 = gr.Dropdown(
                            label="任务", choices=["VQA", "Detection", "Segmentation", 
                                                "Grounding Detection", "Grounding Segmentation", 
                                                "Gcg Detection", "Gcg Segmentation"],
                            value='VQA',info='请选择你要进行的视觉任务')                
                        input_text_1 = gr.Textbox(label="开始对话",placeholder='输入信息')
                        submit_button_1 = gr.Button("提交", variant="primary")
            ...
            with gr.TabItem("多模态交互-点交互"):
                with gr.Row():
                    with gr.Column():
                        input_img_2 = gr.Image(type="numpy", label="输入图像", height=550,image_mode='RGB')
                        radio_2 = gr.Dropdown(label="任务", choices=["VQA","Grounding Detection", 
                                                                    "Grounding Segmentation"],
                                                value='VQA',info='请选择你要进行的视觉任务')
                        input_text_2 = gr.Textbox(label="开始对话",placeholder='输入信息') 
                        submit_button_2 = gr.Button("提交", variant="primary")
            ...
            with gr.TabItem("多模态交互-线交互"):
                with gr.Row():
                    with gr.Column():
                        # input_img_4 = gr.ImageEditor(type="numpy",tool="sketch", brush_radius=5,
                        #                         source="upload", label="输入图像", height=550)
                        input_img_4 = gr.Sketchpad(type='numpy',sources="upload", label="输入图像", height=550 ,image_mode='RGB',layers=False)
                        radio_4 = gr.Dropdown(label="任务", choices=["VQA","Grounding Detection", 
                                                                    "Grounding Segmentation"],
                                            value='VQA',info='请选择你要进行的视觉任务')
                        input_text_4 = gr.Textbox(label="开始对话",placeholder='输入信息')
                        submit_button_4 = gr.Button("提交", variant="primary")                      
        ...
        with gr.Column():
            ...
            with gr.Row():
                with gr.Column():
                    chatbot = gr.Chatbot(height=600,label="聊天内容")
                    output_image = gr.Image(label="输出图像", height=336, interactive=False)             
                    clear_button = gr.Button("🗑 清除历史信息")
    ...
    clear_button.click(clear_states,
        ...
        None,
        None,
        js="() => {document.body.innerHTML='<h1 style=\"font-family:monospace;margin-top:20%;color:lightgray;text-align:center;\">Reloading...</h1>'; setTimeout(function(){location.reload()},2000); return []}",
    )
